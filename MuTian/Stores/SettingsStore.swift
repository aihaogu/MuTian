...
final class SettingsStore: ObservableObject {
    ...
    init() {
        if let data = try? Data(contentsOf: AppPaths.settingsFile),
           let decoded = try? JSONDecoder.muTian.decode(AppSettings.self, from: data) {
            var migrated = decoded
            migrated.databaseDirectory = AppPaths.supportDirectory.path
            if !FileManager.default.isExecutableFile(atPath: migrated.bookgetExecutablePath) {
    ...
    private func save() {
        guard let data = try? JSONEncoder.muTian.encode(settings) else { return }
        try? data.write(to: AppPaths.settingsFile, options: [.atomic])
    }
}

extension JSONEncoder {
    static var muTian: JSONEncoder {
